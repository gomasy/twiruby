...
describe TwiRuby::REST::Favorites do
  ...
  describe "#favorites_destroy" do
    before do
      stub_post("/1.1/favorites/destroy.json?id=20303971").to_return(:status => 200, :headers => { "content-type" => "application/json;utf-8" }, :body => fixtures("status.json"))
    end
  ...
  describe "#favorites_create" do
    before do
      stub_post("/1.1/favorites/create.json?id=20303971").to_return(:status => 200, :headers => { "content-type" => "application/json;utf-8" }, :body => fixtures("status.json"))
    end
